...
if (isset($_GET['column'])) { // 컬럼 삭제
    $query = "ALTER TABLE `phpdaelim4`.`".$tablename."` DROP COLUMN `".$_GET['column']."`;";
    // echo $query;
    $result = mysqli_query($db0, $query);
} else 
if (isset($_GET['mode']) && $_GET['mode'] == "new"){ // 컬럼 삽입
    $query = "ALTER TABLE `phpdaelim4`.`".$tablename."` ADD COLUMN `".$_POST['fieldname']."` VARCHAR(45) NULL;";
    $result = mysqli_query($db0, $query);
}
echo "<a href='/'>목록</a>";
